...
import { getIpfsProtocolUrl, getIpfsUrl } from "@/utils/ipfs";
...
export default function LocationForm({ onSuccess, initialData }: LocationFormProps) {
  ...
  const { locations: existingLocations, isLoading: locationsLoading } = useLocationRegistry();
  ...
  const handleImageUpload = (data: { ipfsHash: string; ipfsUrl: string; fileType: "image" | "video" }) => {
    ...
    setGeneratedImagePreview(data.ipfsUrl); // Use the HTTP gateway URL for preview
    setFormData((prev) => ({
      ...prev,
      sceneURI: ipfsUrl,
    }));
  ...
  const handleGenerateImage = async () => {
    if (!formData.displayName || !formData.description) {
      setError("Please fill in the location name and description before generating an image.");
      return;
    }
    ...
    try {
      ...
      if (!data.imageIpfsHash || !data.imageIpfsUrl) {
        throw new Error("Invalid response from image generation API");
      }

      const imageUrl = getIpfsProtocolUrl(data.imageIpfsHash);
      // Use the preview URL from API if available, otherwise generate it
      const previewUrl = data.imagePreviewUrl || getIpfsUrl(data.imageIpfsHash);
      setUploadedImageHash(data.imageIpfsHash);
      ...
      setError(error instanceof Error ? error.message : "Failed to generate image. You can manually upload an image instead.");
    } finally {
  ...
  return (
    <FormContainer>
      <form onSubmit={handleSubmit}>
        ...
        <FormGroup>
          <Label htmlFor="parentLocationId">
            Parent Location (must be visited before this location unlocks)
          </Label>
          <Select
            id="parentLocationId"
            ...
            onChange={handleChange}
            required
            disabled={isCreating || success || locationsLoading}
          >
            <option value="0">None (Root Location)</option>
            {existingLocations
              .filter((loc) => loc.id !== BigInt(initialData?.parentLocationId || 0)) // Don't show self as parent
              .map((location) => (
                <option key={location.id.toString()} value={location.id.toString()}>
                  {location.displayName} (ID: {location.id.toString()})
                </option>
              ))}
          </Select>
          {formData.parentLocationId !== 0 && (
            <p style={{ color: colors.textMuted, fontSize: "0.85rem", marginTop: "0.5rem" }}>
              Players must visit the parent location before this location becomes available.
            </p>
          )}
        </FormGroup>
        ...
        <FormGroup>
          ...
          <div style={{ display: "flex", gap: "1rem", marginBottom: "1rem", flexWrap: "wrap" }}>
            <Button
              ...
              disabled={isCreating || success || isGeneratingImage || !formData.displayName || !formData.description}
            ...
            <div style={{ 
              ...
              {isGeneratingImage && (
                <span>Creating a unique image based on your location details and the island's mythos style...</span>
              )}
            </div>
          </div>
          ...
          {generatedImagePreview && (
            <ImagePreview>
              <PreviewImage 
                ...
                alt="Generated location image"
                onError={(e) => {
                  // Fallback to Pinata gateway if first one fails
                  const target = e.target as HTMLImageElement;
                  if (uploadedImageHash) {
                    target.src = `https://gateway.pinata.cloud/ipfs/${uploadedImageHash}`;
                  }
                }}
              />
              <RemovePreviewButton
          ...
          <MediaUpload
            ...
            disabled={isCreating || success || isGeneratingImage}
            label={hasExistingScene ? "Upload new location image (optional)" : generatedImagePreview ? "Or upload a different image" : "Or upload your own location image"}
          />
          <Input
            id="sceneURI"
            name="sceneURI"
            ...
            onChange={handleChange}
            disabled={isCreating || success}
            placeholder="ipfs://Qm... (auto-filled when image is generated/uploaded or if existing scene found)"
